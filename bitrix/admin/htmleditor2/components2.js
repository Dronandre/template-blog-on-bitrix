	...
	BXComponents2Taskbar.prototype.BXShowComponent2Panel = function (_bNew, _pTaskbar, _pElement)
	{
		...
		if (temlLen > 0)
		{
			...
			for (j = 0; j < temlLen; j++)
			{
				...
				bSel = ((arAllProps.template == undefined && (el.name == "_default" || el.name=="")) || arAllProps.template == "" && el.name=="_default" || arAllProps.template == el.name);
...
function __checkComp2Template(params)
{
	...
	for (compName in arComponents2)
	{
		...
		for (i =0; i < len; i++)
		{
			...
			if (!arComp2TemplateLists[name][template] || ((template=="" || template=="_default") && !(arComp2TemplateLists[name][''] || arComp2TemplateLists[name]['_default'])))
			{
				allParams.template = arComp2TemplateLists[name][''] ? "" : "_default";
