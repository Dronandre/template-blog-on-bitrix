	...
	$arFeaturesTitles[$feature_id] = (
		...
		: (
			...
			&& strlen($arMenuTmp["Title"][$feature_id]) > 0
				...
				: GetMessage("SEARCH_CONTENT_TYPE_".strtoupper($feature_id)."_".SONET_ENTITY_GROUP)
		)
	);
}
...
foreach($arResult["SEARCH"] as $i=>$arItem)
{
			...
			elseif(CModule::IncludeModule('iblock'))
			{
				...
				if(substr($arItem["ITEM_ID"], 0, 1) !== "S")
				{
					...
					if (!$file && $element_type && $element_type != "files")
						if(file_exists($abs_path.$element_type.".default.gif"))
							$file = $element_type.".default.gif";
	...
	if(!$file)
	{
		if(file_exists($abs_path.$arItem["MODULE_ID"].".default.gif"))
			$file = $arItem["MODULE_ID"].".default.gif";
		else
			$file = "default.gif";
	}
